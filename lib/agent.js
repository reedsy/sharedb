  ...
  try {
    ...
    switch (request.a) {
      ...
      case 'p':
        ...
        if (!presence) return callback({code: 4000, message: 'Invalid presence message'});
...
Agent.prototype._broadcastPresence = function(collection, id, presence, callback) {
  // TODO: Check if message has been superseded (cf AgentPresence.handlePresenceMessage)
  this._subscribeToPresence(collection, id, function(error) {
...
Agent.prototype._subscribeToPresence = function(collection, id, callback) {
...
Agent.prototype._isPresenceData = function(data) {
  return data.a === 'p';
};

Agent.prototype._handlePresenceData = function(data) {
  // TODO: Add a test case for this
  if (data.src !== this.clientId) {
    var backend = this.backend;
    var agent = this;
    backend.transformPresenceToLatestVersion(agent, data, function(error, presence) {
      if (error) return agent.send({error: error});
      var context = {
        collection: presence.c,
        presence: presence
      };
      // TODO: Think more carefully about what information to send to middleware, and how to handle errors?
      backend.trigger(backend.MIDDLEWARE_ACTIONS.sendPresence, agent, context, function(error) {
        if (error) return agent.send({error: error});
        agent.send(presence);
      });
    });
