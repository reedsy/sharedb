...
// TODO: Write a suite of tests for transformPresence? eg null is a valid presence
// and means no presence
exports.transformPresence = function(presence, op, isOwnOp) {
  var opError = this.checkOp(op);
  if (opError) return error;

  var type = presence.t;
  if (typeof type === 'string') {
    type = types[type];
  }
  // TODO: Update to use string error codes
  if (!type) return {code: 4008, message: 'Unknown type'};
  if (!util.supportsPresence(type)) return {code: 9999, message: 'Type does not support presence'};
