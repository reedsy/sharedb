...
var LocalPresence = require('./presence/local-presence');
var RemotePresence = require('./presence/remote-presence');
...
function Connection(socket) {
  ...
  // Maps from presence ID -> presence objects
  this._localPresences = {};
...
// TODO: Some way of subscribing to presence without emitting?
// TODO: Unsubscribe from presence?
// TODO: Should this return the presence object
Connection.prototype.createPresence = function(collection, id, value, callback) {
  var presenceId = hat();
  var presence = new LocalPresence(this, presenceId, collection, id);
  this._localPresences[presenceId] = presence;
  presence.update(value, {requestPresence: true}, callback);
};

Connection.prototype._getRemotePresence = function(collection, id, presenceId) {
  if (!this._remotePresences[presenceId]) {
    this._remotePresences[presenceId] = new RemotePresence(this, presenceId, collection, id);
  }

  return this._remotePresences[presenceId];
};

Connection.prototype._handlePresence = function(error, message) {
  var localPresence = this._localPresences[message.id];
  if (localPresence) return localPresence._ack(error, message.seq);

  var remotePresence = this._getRemotePresence(message.c, message.d, message.id);
  remotePresence._receiveUpdate(error, message);

  if (message.p == null) {
    remotePresence.remove();
  }

  var presenceResponseRequested = !!message.r;
  if (presenceResponseRequested) {
    this._sendLocalPresences();
  }
};

Connection.prototype._sendLocalPresences = function() {
  Object.keys(this._localPresences).forEach(function(id) {
    var localPresence = this._localPresences[id];
    localPresence.send();
  }.bind(this));
};
